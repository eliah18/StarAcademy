...
$cnn=mysqli_connect('localhost','root','','star') OR exit('Server Down');

$trun=$cnn->query("TRUNCATE TABLE graph");
$select=mysqli_query($cnn,"SELECT DISTINCT(Gender)  FROM  `personal_info`  ");
while ($extract=mysqli_fetch_assoc($select)){
  $sex=$extract['Gender'];
  //Select and count
  $count_sql=mysqli_query($cnn,"SELECT * FROM `personal_info` WHERE Gender='$sex' ");
...
$choose=mysqli_query($cnn,"SELECT DISTINCT(xul_province)  FROM  `xul_info`  ");
while ($pull=mysqli_fetch_assoc($choose)){
 $status=$pull['xul_province'];
 //Select and count
 $count_sqli=mysqli_query($cnn,"SELECT * FROM `xul_info` WHERE xul_province='$status' ");
