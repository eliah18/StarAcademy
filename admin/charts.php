        ...
        <div class="container-fluid">
          ...
          <div class="row">
            ...
            <div class="col-xl-8 col-lg-7">
              ...
              $cnn=mysqli_connect('localhost','root','','expertdb') OR exit('Server Down');
              
             $trun=$cnn->query("TRUNCATE TABLE graph");
             $select=mysqli_query($cnn,"SELECT DISTINCT(Gender)  FROM  `personal_details`  ");
             while ($extract=mysqli_fetch_assoc($select)){
                $sex=$extract['Gender'];
                //Select and count
                $count_sql=mysqli_query($cnn,"SELECT * FROM `personal_details` WHERE Gender='$sex' ");
                $numbers=mysqli_num_rows($count_sql);
                //INSERT INTO TEMPO TABLE
                $create_graph=mysqli_query($cnn,"INSERT INTO `graph`(`graph_id`, `x_axis`, `y_axis`) VALUES ('1','$sex','$numbers') ");
             }
             ?>
             <?php
             
             $trunc=$cnn->query("TRUNCATE TABLE graph2");
             $choose=mysqli_query($cnn,"SELECT DISTINCT(V_state)  FROM  `secondary_personal_details`  ");
             while ($pull=mysqli_fetch_assoc($choose)){
               $status=$pull['V_state'];
               //Select and count
               $count_sqli=mysqli_query($cnn,"SELECT * FROM `secondary_personal_details` WHERE V_state='$status' ");
               $number=mysqli_num_rows($count_sqli);
               //INSERT INTO TEMPO TABLE
               $create_graph2=mysqli_query($cnn,"INSERT INTO `graph2`(`id`, `x_axis`, `y_axis`) VALUES ('1','$status','$number') ");
             }
             ?> 
      ...
      var myBar = new Chart(ctx, {
	   ...
	   ?>
        data: {
            labels: [
              <?php				
$sqlx2 = mysqli_query($cnn,"select * from  graph2 ");//This is x-axis blessings
while($row2 = mysqli_fetch_array($sqlx2))
{
$mynum2=$row2['x_axis'];
//$array21['test']= $row1['firstname'];
//echo "".$mynum1.",";
echo "'".$mynum2."',";
      ...
      var myChart = new Chart(ctx, {
	   ...
	   ?>
        data: {
            labels: [
              <?php				
$sqlx1 = mysqli_query($cnn,"select * from  graph ");//This is x-axis blessings
